...
export const surveysApi = {
  ...
  createQuestion: async (payload: Pick<SurveyQuestion, 'question' | 'sort_order' | 'is_active'>) => {
    const { data } = await adminClient.post('/api/v1/surveys/questions', payload);
    return data as SurveyQuestion;
  },
  updateQuestion: async (id: number, payload: Pick<SurveyQuestion, 'question' | 'sort_order' | 'is_active'>) => {
